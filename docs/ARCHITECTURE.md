...
Local git changed files are a fallback for local-only metadata; a base-branch diff supplements the list only when changed-file metadata is explicitly partial or unavailable.
...
If the gate artifact is missing, unreadable, or already `ERROR`, the command writes `ERROR` draft artifacts that explain analysis could not complete and avoids making a merge-safety judgment.
